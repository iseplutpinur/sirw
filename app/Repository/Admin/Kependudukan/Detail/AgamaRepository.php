...
use App\Models\DataMaster\Agama as DataMasterAgama;
...
class AgamaRepository extends Controller
{
    ...
    public function update(Request $req)
    {
        try {
            ...
            $model = PendudukAgama::find($req->id);
            $model->penduduk_id = $req->penduduk_id;
            $model->dari = $req->dari;
            $model->sampai = $req->sampai;
            $model->agama_id = $req->agama_id;
    ...
    public function refresh(?int $penduduk_id)
    {
        ...
        $rows = PendudukAgama::where('penduduk_id', '=', $penduduk_id)->orderBy('dari')->get();
    ...
    {
        // table penduduk
        $tp_model = PendudukAgama::tableName;
        $t_model = DataMasterAgama::tableName;
        ...
        $model = PendudukAgama::select(array_merge([
            "$tp_model.id",
            ...
            ->leftJoin($t_model, "$t_model.id", "=", "$tp_model.agama_id")
