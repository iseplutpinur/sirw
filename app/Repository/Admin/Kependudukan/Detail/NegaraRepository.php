...
class NegaraRepository extends Controller
{
    ...
    public function update(Request $req)
    {
        try {
            ...
            $model = PendudukNegara::find($req->id);
            $model->penduduk_id = $req->penduduk_id;
            $model->dari = $req->dari;
            $model->sampai = $req->sampai;
            $model->negara = $req->negara;
    ...
    public function delete(PendudukNegara $model)
    {
        try {
            DB::beginTransaction();
            $model->delete();
    ...
    public function refresh(?int $penduduk_id)
    {
        ...
        $rows = PendudukNegara::where('penduduk_id', '=', $penduduk_id)->orderBy('dari')->get();
    ...
    {
        // table penduduk
        $tp_model = PendudukNegara::tableName;
        ...
        $c_negara_str = 'negara_str';
        $this->query[$c_negara_str] = <<<SQL
            (if($tp_model.negara = 1, 'WNI', 'WNA') )
        SQL;
        $this->query["{$c_negara_str}_alias"] = $c_negara_str;

        $c_negara_str_full = 'negara_str_full';
        $this->query[$c_negara_str_full] = <<<SQL
            (if($tp_model.negara = 1, 'Warga Negara Indonesia', 'Warga Negara Asing') )
        SQL;
        $this->query["{$c_negara_str_full}_alias"] = $c_negara_str_full;
        ...
        $model = PendudukNegara::select(array_merge([
            "$tp_model.id",
            "$tp_model.penduduk_id",
            "$tp_model.negara",
            "$tp_model.negara_nama",
            "$tp_model.dari",
            "$tp_model.sampai",
            "$tp_model.created_at",
            "$tp_model.updated_at",
