...
class RtRepository extends Controller
{
    ...
    public function delete(PendudukRt $model)
    {
        try {
            DB::beginTransaction();
            $model->delete();
    ...
    public function refresh(?int $penduduk_id)
    {
        ...
        $rows = PendudukRt::where('penduduk_id', '=', $penduduk_id)->orderBy('dari')->get();
    ...
    private function q(?array $filter = [], bool $result = true)
    {
        ...
        $model = PendudukRt::select(array_merge([
            "$tp_model.id",
