...
use App\Models\DataMaster\StatusPenduduk as DataMasterStatusPenduduk;
...
class StatusPendudukRepository extends Controller
{
    ...
    public function update(Request $req)
    {
        try {
            ...
            $model = PendudukStatus::find($req->id);
            $model->penduduk_id = $req->penduduk_id;
            $model->dari = $req->dari;
            $model->sampai = $req->sampai;
            $model->status_penduduk_id = $req->status_penduduk_id;
    ...
    {
        // table penduduk
        $tp_model = PendudukStatus::tableName;
        $t_model = DataMasterStatusPenduduk::tableName;
        ...
        $model = PendudukStatus::select(array_merge([
            "$tp_model.id",
            "$tp_model.penduduk_id",
            "$tp_model.status_penduduk_id",
            "$tp_model.dari",
            "$tp_model.sampai",
            "$tp_model.created_at",
            "$tp_model.updated_at",
            ...
            ->leftJoin($t_model, "$t_model.id", "=", "$tp_model.status_penduduk_id")
    ...
    public function refresh(?int $penduduk_id)
    {
        ...
        $rows = PendudukStatus::where('penduduk_id', '=', $penduduk_id)->orderBy('dari')->get();
