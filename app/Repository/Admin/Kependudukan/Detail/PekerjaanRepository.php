...
use App\Models\DataMaster\Pekerjaan as DataMasterPekerjaan;
...
class PekerjaanRepository extends Controller
{
    ...
    public function update(Request $req)
    {
        try {
            ...
            $model = PendudukPekerjaan::find($req->id);
            $model->penduduk_id = $req->penduduk_id;
            $model->dari = $req->dari;
            $model->sampai = $req->sampai;
            $model->pekerjaan_id = $req->pekerjaan_id;
    ...
    public function delete(PendudukPekerjaan $model)
    {
        try {
            DB::beginTransaction();
            $model->delete();
    ...
    {
        // table penduduk
        $tp_model = PendudukPekerjaan::tableName;
        $t_model = DataMasterPekerjaan::tableName;
        ...
        $model = PendudukPekerjaan::select(array_merge([
            "$tp_model.id",
            "$tp_model.penduduk_id",
            "$tp_model.pekerjaan_id",
            "$tp_model.dari",
            "$tp_model.sampai",
            "$tp_model.created_at",
            "$tp_model.updated_at",
            ...
            ->leftJoin($t_model, "$t_model.id", "=", "$tp_model.pekerjaan_id")
    ...
    public function refresh(?int $penduduk_id)
    {
        ...
        $rows = PendudukPekerjaan::where('penduduk_id', '=', $penduduk_id)->orderBy('dari')->get();
