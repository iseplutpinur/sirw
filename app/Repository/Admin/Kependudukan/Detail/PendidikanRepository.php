...
use App\Models\DataMaster\Pendidikan as DataMasterPendidikan;
...
class PendidikanRepository extends Controller
{
    ...
    public function insert(Request $req)
    {
        try {
            ...
            $check = PendudukPendidikan::where('penduduk_id', '=', $req->penduduk_id)
                ->where('dari', '=', $req->dari)->count();

            if ($check) throw new Exception("Tanggal dari sudah digunakan");
            $model = new PendudukPendidikan();
    ...
    public function update(Request $req)
    {
        try {
            ...
            $model = PendudukPendidikan::find($req->id);
            $model->penduduk_id = $req->penduduk_id;
            $model->dari = $req->dari;
            $model->sampai = $req->sampai;
            $model->pendidikan_id = $req->pendidikan_id;
    ...
    {
        // table penduduk
        $tp_model = PendudukPendidikan::tableName;
        $t_model = DataMasterPendidikan::tableName;
        ...
        $model = PendudukPendidikan::select(array_merge([
            "$tp_model.id",
            "$tp_model.penduduk_id",
            "$tp_model.pendidikan_id",
            "$tp_model.dari",
            "$tp_model.sampai",
            "$tp_model.created_at",
            "$tp_model.updated_at",
            ...
            ->leftJoin($t_model, "$t_model.id", "=", "$tp_model.pendidikan_id")
    ...
    public function refresh(?int $penduduk_id)
    {
        ...
        $rows = PendudukPendidikan::where('penduduk_id', '=', $penduduk_id)->orderBy('dari')->get();
