...
class PekerjaanController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (request()->ajax()) {
            ...
            // get data from model
            $model = Pekerjaan::select(array_merge(['id'], $select))
        ...
        $page_attr = [
            'title' => 'Manage List Pekerjaan',
            'breadcrumbs' => [
                ['name' => 'Dashboard'],
                ['name' => 'Data Master'],
            ]
        ];
        return view('admin.data_master.pekerjaan', compact('page_attr'));
    }
    ...
    {
        try {
            $model = Pekerjaan::find($request->id);
    ...
    public function delete(Pekerjaan $model)
    {
        try {
            $model->delete();
            return response()->json();
    ...
    {
        try {
            $model = Pekerjaan::select(['id', DB::raw('nama as text')])
                ->whereRaw("(`nama` like '%$request->search%' or `id` like '%$request->search%')")
                ->limit(10);

            $result = $model->get()->toArray();
            if ($request->with_empty && $request->search) {
                $result = array_merge([['id' => $request->search, 'text' => $request->search . ' (Buat Data Pekerjaan)']], $result);
            }
