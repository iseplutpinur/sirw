...
class PendidikanController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (request()->ajax()) {
            ...
            // get data from model
            $model = Pendidikan::select(array_merge(['id'], $select))
        ...
        $page_attr = [
            'title' => 'Manage List Pendidikan',
            'breadcrumbs' => [
                ['name' => 'Dashboard'],
                ['name' => 'Data Master'],
            ]
        ];
        return view('admin.data_master.pendidikan', compact('page_attr'));
    }
    ...
    public function delete(Pendidikan $model)
    {
        try {
            $model->delete();
            return response()->json();
    ...
    {
        try {
            $model = Pendidikan::select(['id', DB::raw('nama as text')])
                ->whereRaw("(`nama` like '%$request->search%' or `id` like '%$request->search%')")
                ->limit(10);

            $result = $model->get()->toArray();
            if ($request->with_empty && $request->search) {
                $result = array_merge([['id' => $request->search, 'text' => $request->search . ' (Buat Data Pendidikan)']], $result);
            }
