...
class StatusPendudukController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (request()->ajax()) {
            ...
            // get data from model
            $model = StatusPenduduk::select(array_merge(['id'], $select))
        ...
        $page_attr = [
            'title' => 'Manage List StatusPenduduk',
            'breadcrumbs' => [
                ['name' => 'Dashboard'],
                ['name' => 'Data Master'],
            ]
        ];
        return view('admin.data_master.status_penduduk', compact('page_attr'));
    }
    ...
    {
        try {
            $model = StatusPenduduk::find($request->id);
    ...
    public function delete(StatusPenduduk $model)
    {
        try {
            $model->delete();
            return response()->json();
    ...
    {
        try {
            $model = StatusPenduduk::select(['id', DB::raw('nama as text')])
                ->whereRaw("(`nama` like '%$request->search%' or `id` like '%$request->search%')")
                ->limit(10);

            $result = $model->get()->toArray();
            if ($request->with_empty && $request->search) {
                $result = array_merge([['id' => $request->search, 'text' => $request->search . ' (Buat Data StatusPenduduk)']], $result);
            }
