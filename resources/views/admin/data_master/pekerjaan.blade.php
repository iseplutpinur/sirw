    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const new_table = table_html.DataTable({
                ...
                type: 'GET',
                ajax: {
                    url: "{{ route('admin.data_master.pekerjaan') }}",
                    data: function(d) {
            ...
            $('#MainForm').submit(function(e) {
                ...
                const route = ($('#id').val() == '') ?
                    "{{ route('admin.data_master.pekerjaan.insert') }}" :
                    "{{ route('admin.data_master.pekerjaan.update') }}";
                $.ajax({
                    type: "POST",
                    url: route,
                    headers: {
        ...
        function add() {
            ...
            $('#modal-default-title').html("Add Pekerjaan");
        ...
        function editFunc(datas) {
            ...
            $('#modal-default-title').html("Edit Pekerjaan");
            $('#modal-default').modal('show');
            $('#MainForm').trigger("reset");
            $('#id').val(data.id);
            $('#nama').val(data.nama);
        ...
        function deleteFunc(id) {
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: `{{ url('admin/data_master/pekerjaan') }}/${id}`,
                        type: 'DELETE',
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        ...
                        success: function(data) {
                            Swal.fire({
                                position: 'center',
                                icon: 'success',
                                title: 'Pekerjaan  deleted successfully',
                                showConfirmButton: false,
                                timer: 1500
                            })
                            var oTable = table_html.dataTable();
