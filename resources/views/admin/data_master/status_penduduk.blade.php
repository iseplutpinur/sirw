        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-md-flex flex-row justify-content-between">
                    <h3 class="card-title">Status Penduduk</h3>
                ...
                <div class="card-body">
                    ...
                    <form action="javascript:void(0)" class="form-inline ml-md-3 mb-md-3" id="FilterForm">
                        <div class="form-group me-md-3">
                            <label for="filter_status">Status Penduduk</label>
                            <select class="form-control" id="filter_status" name="filter_status" style="max-width: 200px">
                                <option value="">All Status Penduduk</option>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const new_table = table_html.DataTable({
                ...
                type: 'GET',
                ajax: {
                    url: "{{ route('admin.data_master.status_penduduk') }}",
                    data: function(d) {
            ...
            $('#MainForm').submit(function(e) {
                ...
                const route = ($('#id').val() == '') ?
                    "{{ route('admin.data_master.status_penduduk.insert') }}" :
                    "{{ route('admin.data_master.status_penduduk.update') }}";
                $.ajax({
                    type: "POST",
                    url: route,
                    headers: {
        ...
        function add() {
            ...
            $('#modal-default-title').html("Add Status Penduduk");
        ...
        function editFunc(datas) {
            ...
            $('#modal-default-title').html("Edit Status Penduduk");
            $('#modal-default').modal('show');
            $('#MainForm').trigger("reset");
            $('#id').val(data.id);
            $('#nama').val(data.nama);
        ...
        function deleteFunc(id) {
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: `{{ url('admin/data_master/status_penduduk') }}/${id}`,
                        type: 'DELETE',
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        ...
                        success: function(data) {
                            Swal.fire({
                                position: 'center',
                                icon: 'success',
                                title: 'Status Penduduk  deleted successfully',
                                showConfirmButton: false,
                                timer: 1500
                            })
                            var oTable = table_html.dataTable();
