        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header d-md-flex flex-row justify-content-between">
                    <h3 class="card-title">Status Kawin</h3>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const new_table = table_html.DataTable({
                ...
                type: 'GET',
                ajax: {
                    url: "{{ route('admin.data_master.status_kawin') }}",
                    data: function(d) {
            ...
            $('#MainForm').submit(function(e) {
                ...
                const route = ($('#id').val() == '') ?
                    "{{ route('admin.data_master.status_kawin.insert') }}" :
                    "{{ route('admin.data_master.status_kawin.update') }}";
                $.ajax({
                    type: "POST",
                    url: route,
                    headers: {
        ...
        function add() {
            ...
            $('#modal-default-title').html("Add Status Kawin");
        ...
        function editFunc(datas) {
            ...
            $('#modal-default-title').html("Edit Status Kawin");
            $('#modal-default').modal('show');
            $('#MainForm').trigger("reset");
            $('#id').val(data.id);
            $('#nama').val(data.nama);
        ...
        function deleteFunc(id) {
            swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: `{{ url('admin/data_master/status_kawin') }}/${id}`,
                        type: 'DELETE',
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        ...
                        success: function(data) {
                            Swal.fire({
                                position: 'center',
                                icon: 'success',
                                title: 'Status Kawin  deleted successfully',
                                showConfirmButton: false,
                                timer: 1500
                            })
                            var oTable = table_html.dataTable();
